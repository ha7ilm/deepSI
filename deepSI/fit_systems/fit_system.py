        ...
        try:
            ...
            for epoch in (tqdm(epochsrange) if verbose>0 else epochsrange):
                ...
                if epoch%1000==0:
                    ...
                    N_samples = 600
                    N_samples_cutoff = 0
                    current_val_nsn = self.cal_validation_error(val_sys_data[N_samples_cutoff:N_samples-N_samples_cutoff], validation_measure='15-step-NRMS')
                ...
                if verbose>0:
                    ...
                    if concurrent_val: #if concurrent val than print validation freq
                        ...
                        valfeqstr = f''
                    trainstr = f'sqrt loss {train_loss_epoch**0.5:7.10}' if sqrt_train and train_loss_epoch>=0 else f'loss {train_loss_epoch:7.10}'
                    Loss_val_now = self.Loss_val[-1] if len(self.Loss_val)!=0 else float('nan')
                    Loss_str = f'Epoch {epoch+1:4}, {trainstr}, Val {validation_measure} {Loss_val_now:6.10} (Best {self.bestfit:6.10} @ {best_epoch})'
